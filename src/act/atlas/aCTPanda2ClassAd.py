...
class aCTPanda2ClassAd:
    ...
    def setInputs(self):
        ...
        self.classad['transfer_input_files'] = str(tmpfile)

    def setLog(self):

        self.classad['Log'] = str(os.path.join(self.logdir, '$(Cluster).$(Process).log'))

    def setOutputs(self):

        self.classad['Output'] = str(os.path.join(self.logdir, '$(Cluster).$(Process).out'))
        self.classad['Err'] = str(os.path.join(self.logdir, '$(Cluster).$(Process).err'))
...
if __name__ == '__main__':
    ...
    info = {'schedconfig': 'ANALY_SiGNET_DIRECT', 'corecount': 1, 'truepilot': True, 'maxwalltime': 10800, 'direct_access_lan': True, 'type': 'analysis'}
    conf = aCTConfigATLAS()
    a = aCTPanda2ClassAd(pjob, 'ANALY_SiGNET_DIRECT', info, '/tmp', conf, log)
    a.parse()
    print a.getClassAd()
